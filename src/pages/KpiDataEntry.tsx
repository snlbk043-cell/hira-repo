...
import { fmtPercent, fmtUnitValue } from '../lib/format';
import type { ActionStatus, Direction, Aggregation, KpiDefinition, PQSDC, Priority } from '../types';

export function KpiDataEntry() {
  const { state, filters, setFilters, updateDay, updateRecord, addKpi, removeKpi } = useAppStore();
  ...
  return (
    <div className="flex flex-col gap-5 pb-10">
                  ...
                  <tr className="border-t" style={{ borderColor: 'var(--border)' }}>
                    ...
                    <Td>
                      <button
                        type="button"
                        onClick={() => {
                          if (confirm(`Remove ${k.def.name}?`)) removeKpi(k.def.kpiId);
                        }}
      ...
      {showAdd && <AddKpiModal onClose={() => setShowAdd(false)} onAdd={addKpi} existingIds={state.kpiDefinitions.map((d) => d.kpiId)} />}
    </div>
  );
